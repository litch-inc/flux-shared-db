  ...
  constructor(opts) {
    try {
      ...
      this.socket.on("error", (error) => {
        log.error(`>> ${error}`, { label: 'mysqlServer - Server - constructor - try - socket.on - error' });
      ...
      log.error(`>> ${error}`, { label: 'mysqlServer - Server - constructor - catch - error' });
    }
  }
   ...
   try {
     ...
     log.error(`>> ${error}`, { label: 'mysqlServer - Server - end - catch - error' });
   }
 };
 ...
 sendDefinitions(definitions) {
  ...
  handleData = (data) => {
    try {
      ...
      log.error(`>> ${error}`, { label: 'mysqlServer - Server - handleData - catch - error' });
    }
  }
  ...
  gatherIncoming() {
    try {
      ...
      log.error(`>> ${error}`, { label: 'mysqlServer - Server - gatherIncoming - catch - error' });
    }
  }
  ...
  helloPacketHandler = (packet) => {
    ...
    if (packet.length == 0) {
      return this.sendError({ message: "Zero length hello packet" });
    }
    let ptr = 0;
    ...
    return Promise.resolve(this.onAuthorize({ clientFlags, maxPacketSize, username, database, remoteIP}))
      ...
      .catch((error) => {
        log.error(`>> ${error}`, { label: 'mysqlServer - Server - helloPacketHandler - onAuthorize - catch - error' });
        this.socket.destroy();
      });
  }
