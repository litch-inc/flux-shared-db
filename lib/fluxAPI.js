...
async function getApplicationSpecs(appName) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - getApplicationSpecs - catch - error' });
    return [];
  }
}
...
async function getApplicationOwner(appName) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - getApplicationOwner - catch - error' });
    return [];
  }
}
/**
...
async function getApplicationIP(appName) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - getApplicationIP - catch - error' });
    return [];
  }
}
  ...
  try {
    ...
    if (appList.status === 'success') {
      ...
      for (let i = 0; i < appList.data.length; i += 1) {
        if (appList.data[i].Names[0].endsWith(`_${appName}`) && appList.data[i].State !== 'running') {
          isValid = false; break;
        }
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - validateApp - catch - error' });
    return false;
  }
}
...
async function getMaster(ip, port) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - getMaster - catch - error' });
    return [];
  }
}
...
async function getMyIp(ip, port) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - getMyIp - catch - error' });
    return null;
  }
}
...
async function getStatus(ip, port) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - getStatus - catch - error' });
    return null;
  }
}
...
async function resetMaster(ip, port) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - resetMaster - catch - error' });
    return null;
  }
}
...
async function getBackLog(index, socket) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - getBackLog - catch - error' });
    return null;
  }
}
/**
...
async function askQuery(index, socket) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - askQuery - catch - error' });
    return null;
  }
}
...
async function shareKeys(pubKey, socket) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - shareKeys - catch - error' });
    return null;
  }
}
...
async function updateKey(key, value, socket) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - updateKey - catch - error' });
    return null;
  }
}
...
async function getKeys(socket) {
  try {
    ...
    log.error(`>> ${error}`, { label: 'fluxAPI - getKeys - catch - error' });
    return null;
  }
}
