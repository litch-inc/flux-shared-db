  ...
  return __awaiter(this, void 0, void 0, function* () {
    ...
    // mysql2 returns an array of arrays which will all have our one row
      ...
      .map((res, i) => {
        ...
        if (isCreateView(res)) {
          return { ...table, name: res.View, schema: format$$1(res['Create View']), data: null, isView: true };
        }
        return { ...table, name: res.Table, schema: format$$1(res['Create Table']), data: null, isView: false };
      })
      .map((s) => {
...
function compressFile(filename) {
  ...
  const deleteFile = (file) => {
    try {
      ...
      log.error(`>> ${error}`, { label: 'es - compressFile - deleteFile - try - catch - error' });
    }
  };
  try {
