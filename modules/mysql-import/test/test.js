...
const {
  query, mysqlConnect, createTestDB, destroyTestDB, closeConnection,
...
let fs; let MySQLImport; let importer; let
  start_time;
...
describe('Running All Tests', () => {
  before(async () => {
    ...
    importer.onProgress((progress) => {
      ...
      const message = `\tFile ${progress.file_no} of ${progress.total_files}: `
  ...
  it('Test fake sql file.', async () => {
    const fake_sql_file = `${__dirname}/sample_dump_files/more_sample_files/not_sql.txt`;
    let error;
    try {
  ...
  it('Test importing broken file.', async () => {
    const fake_sql_file = `${__dirname}/broken_dump_files/dump.sql`;
    const fake_sql_file2 = `${__dirname}/broken_dump_files/dump_1.sql`;
    let error;
    try {
